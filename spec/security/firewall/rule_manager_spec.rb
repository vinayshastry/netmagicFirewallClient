...
describe "RuleManager" do
  ...
  it "syncs by adding new rules with firewall client" do
    ...
    add_rule_response = "<Response success='Success'></Response>"

    expect(nm_client_mock).to receive('get_rules').at_least(:once).and_return([])

    expect(nm_client_mock).to receive('add_rule').with(src_zone, src_ip, dest_zone, dest_ip, 123, 2001).and_return(add_rule_response)
    expect(nm_client_mock).to receive('add_rule').with(src_zone, src_ip, dest_zone, dest_ip, 687, 2002).and_return(add_rule_response)
    expect(nm_client_mock).to receive('add_rule').with("zone3", "10.11.12.13", "zone4", "20.21.22.23", 234, 2001).and_return(add_rule_response)
  ...
  it "syncs by removing obsolete rules with firewall client" do
    ...
    expect(nm_client_mock).to receive('get_rules').at_least(:once).and_return([already_existing_rule, obsolete_rule])

    expect(nm_client_mock).to receive('add_rule').with(src_zone, src_ip, dest_zone, "10.12.213.2", 456, 2002).and_return(add_rule_response)
    expect(nm_client_mock).to receive('delete_rule').with(2002, "zone3", "zone4").and_return(add_rule_response)
